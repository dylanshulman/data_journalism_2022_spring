...
international_migration <- read_csv("data/tabula-md-statistical-handbook.csv", skip = 2, col_names = c("state_jurisdiction", "july2010_july2011", "july2011_july2012", "july2012_july2013", "july2013_july2014", "july2014_july2015", "july2015_july2016", "july2016_july2017", "july2017_july2018", "july2018_july2019", "totals"))
```
...
**A1** Suburban Washington Region accounted for 62% of Maryland's international migration, and Montgomery county accounted for 36%. 

```{r}
# Write the code to produce your answer here
international_migration <- international_migration %>% 
  mutate(percent_migration = totals / 198996 * 100) 
international_migration %>% 
  arrange(desc(percent_migration)) %>% 
  select(state_jurisdiction, totals, percent_migration)
```

**Q2** Write a sentence or two that describes the data you produced in A1. Try to convey the most important idea.
**A2** The results show that a majority of people that moved into Maryland from out of the country moved into the Suburban Washington Region, more specifically Montgomery County. 
...
**A3** Southern Maryland Region had a percent change of -58.5%, and Calvert changed -76.9%. 

```{r}
# Write the code to produce your answer here
international_migration %>% 
  mutate(percent_change = (july2017_july2018 - july2016_july2017) / july2016_july2017 * 100) %>% arrange(percent_change) %>% 
  select(state_jurisdiction, july2016_july2017, july2017_july2018, percent_change)
```

**Q4** What's your best guess as to why these changes occurred, and in those places in particular?
**A4** I wonder if there was major flooding in the Southern Part of Maryland, especially in Calvert as it's right on the Bay. Or it could have been Trump's aggressive anti-immigrant rhetoric and policies. 
...
```{r}
# Write the code to load the CSV file here
domestic_migration <- read_csv("data/tabula-md-statistical-handbook-domestic.csv", skip = 2, col_names = c("state_jurisdiction", "july2010_july2011", "july2011_july2012", "july2012_july2013", "july2013_july2014", "july2014_july2015", "july2015_july2016", "july2016_july2017", "july2017_july2018", "july2018_july2019", "totals"))
```
...
**A5** Baltimore City saw the largest decrease in domestic migration at -62,834. 

```{r}
# Write the code to produce your answer here
domestic_migration %>% 
  arrange(totals) %>% 
  select(state_jurisdiction, totals)
```
...
**A6** 14 regions and jurisdictions had net positive migrations for those years. 

```{r}
# Write the code to produce your answer here
domestic_migration %>% 
  filter(july2016_july2017 > 0 & july2017_july2018 > 0 & july2018_july2019 > 0) %>% 
  nrow()
```

**Q7** How would you describe this data? Is there a county or region that stands out, and why?
**A7** I think it's interesting that internationally the most people migrated to the Suburban Washington Region while domestically the most people left that area. Does this mean the area is diversifying? Why does the data point to complete polar opposites depending on whether people moved domestically or internationally? I would be curious to see the demographics of the area over this time frame. 