...
thirty_days <- crime %>% group_by(date) %>% arrange(desc(date)) %>% select(date) %>% head(30
thirty_days <- crime %>% filter(date == c(thirty_days))
  
  interval(date, (date == latest_date$date) - 30))

latest_date <- ranked %>% filter(rank == 1)
