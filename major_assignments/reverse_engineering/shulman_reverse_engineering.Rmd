...
* **Analysis summary**: I was able to calculate this ratio with a simple equation after narrowing down the dataset to individual officers. With a clean dataset like this, it was a simple and straightforward process. 
...
# Claim 1, A dozen high ranking officers have three or more complaints; 10 are white 
data %>% 
  filter(rank_now == "Chiefs and other ranks" | rank_now == "Deputy Inspector" | rank_now == "Inspector") %>% 
  select(unique_mos_id, complaint_id) %>% 
  distinct() %>% 
  group_by(unique_mos_id) %>% 
  summarise(count = n()) %>% 
  filter(count > 3) %>% 
