...
**Answer**
Looks like a dataframe of politicians on Twitter and their tweets about coronavirus, including their position in office and the datetime of the tweet. 
```{r}
...
**Answer**
It standardized everything, caps and punctuation, and created a massive list of unique words. 
```{r}
...
**Answer**
A lot of filler words like prepositions and articles. "Covid" is pretty high on the list however. 
...
**Answer**
Yes! There are more specific words that actually tell us something. 
```{r}
unique_words %>%
...
**Answer**
Vaccines have butt their way into the dialogue on coronavirus on Twitter. 
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Some interesting entries, health care and public health being high is a little different than the single phrases. 
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
Again, vaccines are intruding on the data. 
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Dems focus on social distancing, but Republicans on the American people and the virus. Dems posted way more than Republicans. 
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
What stuck out to me is the number four bigram on the republican candidates: "covid positive." Were they announcing positive results?
```{r}
covid_tweets %>%
...
**Answer**
Trump is positive... but I realize it's interpreting the word and not the man. From what I looked at, most of the sentiments seem pretty spot on. 
```{r}
