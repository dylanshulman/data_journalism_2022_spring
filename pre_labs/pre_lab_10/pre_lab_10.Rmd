...
https://datawrapper.dwcdn.net/gIMJj/1/
...
The dataframe includes an NA input and counties outside of MD that have only 1 or so loans. 
...
**Answer** Describe what the code below is doing and the output.
The dataframe now includes only MD counties. 
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
There's now a new column with the full fips code. 
```{r}
...
Fips are now joined to the PPP loan count counties data. 
```{r}
...
Now I'm no longer looking a USA map but a Maryland map. 
...
**Answer** Describe the changes from the previous map
Now instead of the fips code, the labels include the county name and number of counts with commas. 
...
https://datawrapper.dwcdn.net/qACU2/2/