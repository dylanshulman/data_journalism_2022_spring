...
**Answer**
R is taking the PPP data, grouping it by county, finding the total loans in those counties, arranging them in descending order and only showing the top 10. 
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
A bar chart of total loans by county. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
The bars are now ordered in ascending total loans.  
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
It is now a horizontal bar chart. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The grey background is now white. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
There's now a title and better axis labels. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
I see total loans grouped by date approved, saved under ppp_maryland_loans_by_date. 
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
A line chart plotting the total loans across dates. There's a massive spike at the beginning, then radio silence, then a more consistent stream. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
There's a lot more ticks on the x-axis but its such a jumble I can't really tell what's going on. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
The ticks are now set at an angle and are intelligible. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
