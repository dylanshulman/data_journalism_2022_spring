...
**Answer**
amount = <dbl> double, number
name = <chr> character
id = <dbl> double, number
```{r}
...
**Answer**:
There are 192,959 applications with NA, 275 applications with Subway. 
```{r}
...
**Answer**:
There were 135,073 applications with unspecified forgiveness amount. 
```{r}
ppp_maryland_loans %>%
...
**Answer**:
Again, the figure is 135,073. I assume that these applications were not forgiven. 
```{r}
ppp_maryland_loans %>%
...
**Answer**
Uh... 0? September through December 2020 aren't listed.
```{r}
ppp_maryland_loans %>%
...
**Answer**
The maximum amount is \$10 million and the minimum is \$6. Both are suspicious, because I believed that the maximum amount you could receive is \$2 million, and a \$6 loan seems rather ridiculous. 
```{r}
...
**Answer**
The first two rows are duplicates. The column names aren't uniform and street address has a space inside of it. There's also similair addresses that are misspelled or in a variety of formats. Otherwise, there are some issues with structure. Also the ID column is a numeric datatype, although they should be treated as a character. Oppositely, the amount is a character datatype.
```{r}
arnold_md_loans
```
...
**Answer**
It added an x to the front, but it standardized the other column names. 
```{r}
# cleaning function
...
**Answer**
It says there's an invalid type, which I assume means that the amount coloumn is a character type and not a numeric. 
```{r}
# cleaning function
...
**Answer**
There is one set of duplicates, under Stuart Jones LLC. 
```{r}
cleaned_arnold_md_loans %>%
  get_dupes()
```
...
**Answer**
There is now one less row for a total of five. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
