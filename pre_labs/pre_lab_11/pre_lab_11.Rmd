...
**Answer**
A bunch of different file types, all named Hospitals. 
```{r, echo=FALSE}
...
**Answer**
Geographical info, cities, addresses, counties, coordinates but also population, beds and even helipads. 
```{r}
...
**Answer**
The first 10 Maryland hospitals in the set. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Mostly General Acute Care hospitals, some psychiatric and then they become more specialized. 
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
First 10 Maryland General Acute Care Hospitals. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A grid of plotted coordinates in a vague shape of Maryland. 
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Now it's just Maryland counties. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A map of Maryland counties, albeit with the bay filled in. 
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The posititons of hospitals are now layered on top of the Maryland county map. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The MD maps is now white, and the points indicating hospitals are differently colored based off a color code scale depicting how many beds each has. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
I see the biggest cities/counties have the most beds. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Maryland County population dataframe. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The same dataframe, now with a column we can use to make a map. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The names of both sets of counties don't match at all. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The county names have been cleaned to match hospital data. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The two datasets are now joined. 
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A new column has been created with beds per 100k population. 
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A map of Maryland counties, overlaid with a color code matching the number of beds per 100k people. 
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
There is a different color code. 
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
West Maryland is surprisingly well covered by hospital beds, even in comparison to more populated areas. 
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
