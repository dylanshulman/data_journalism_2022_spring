...
**Answer**
Voting data for states alongside business PPE data. 
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
There is a less positive correlation, and a greater spread among the points. 
```{r}
...
**Answer**
R values for the two variables denoting a positive relationship, more so among businesses receiving PPE. 
```{r}
reveal_data %>%
...
**Answer**
The p value is incredibly small, which means it's very likely this is a newsworthy phenomenon and not chance. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
The higher p value means that this could be a symptom of chance. 
```{r}
...
**Answer**
Trump has a higher mean ppp amount per employee and higher mean of businesses receiving ppe. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
I see a t value and a p value in a Welch Two Sample t-test that indicate this may not be noteworthy statistic. 
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
The t value and p value are much stronger here, indicating a possible corellation outside of chance. 
```{r}
